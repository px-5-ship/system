...
setInterval(() => {
  http.get(`http://px--5-sarar-gg-.glitch.me/`);
}, 280000);
...
const youtube = new YouTube("AIzaSyDXCEL2PlY0D400dxebPRAbqk720uEQg7U"); //تعديل اساسي سوي اي بي اي جديد
const hastebins = require("hastebin-gen");
const getYoutubeID = require("get-youtube-id");
const yt_api_key = "AIzaSyDXCEL2PlY0D400dxebPRAbqk720uEQg7U"; ///تعديل اساسي سوي اي بي اي جديد
...
client.on("message", message => {
  if (message.content === "برب") {
    message.channel.send("**:heart:تيت:heart:**");
    message.channel.sendFile("");
  }
});
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "setby")) {
    ...
    client.on("guildMemberAdd", member => {
      ...
      var channel = member.guild.channels.find(gg => gg.name === "🛏رومز"); //// تعديل اساسي
      if (!channel) return;
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "setVc")) {
    ...
    channelfind.setName(
      `فويس اولاين : ${message.guild.members.filter(m => m.voiceChannel).size}` ///تعديل غير اساسي تعديل اسم روم الفويس اونلاين
    );
    message.channel.send("**Done The فويس اولاين  Is Turned On**");
  }
  if (message.content.startsWith(prefix + "vc off")) {
    ...
    message.channel.send("**Done The فويس اولاين Is Turned Off**");
  }
...
let cmds = {
  play: { cmd: "شغل", a: ["p"] },
  skip: { cmd: "تخطى", a: ["s" ] },
  stop: { cmd: "توقف", a: ["ايقاف"] },
  pause: { cmd: "ايقاف مؤقت", a: ["ايقاف مؤقت"] },
  resume: { cmd: "كمل", a: ["r", ] },
  volume: { cmd: "صوت", a: ["vol"] },
  queue: { cmd: "قائمة", a: ["q"] },
  repeat: { cmd: "تكرار", a: ["re"] },
  forceskip: { cmd: "تخطي الكل", a: ["fskip"] },
  skipto: { cmd: "اذهب الى", a: ["st"] },
  nowplaying: { cmd: "الان", a: ["np"] }
};
    ...
    async function handleVideo(video, msg, voiceChannel, playlist = false) {
      ...
      if (!serverQueue) {
        const queueConstruct = {
          ...
          volume: 15, //// تعديل درجة الصوت الاساسية
